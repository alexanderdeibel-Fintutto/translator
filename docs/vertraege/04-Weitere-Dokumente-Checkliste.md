# WEITERE NOTWENDIGE DOKUMENTE & MASSNAHMEN
## Checkliste für die rechtssichere Strukturierung

---

## A. Bereits erstellt (Entwürfe)

- [x] **01 -- Partnerschaftsvorschlag** (1-Seiter für Ulrich)
- [x] **02 -- Lizenzvertrag** (Fintutto UG ↔ AI Tour Guide UG)
- [x] **03 -- Vertriebsvereinbarung** (AI Tour Guide UG ↔ Ulrich Berger)
- [x] **05 -- Ausscheidungsvereinbarung** (Einvernehmliche Trennung von Ulrich Berger)
- [x] **06 -- Anteilsabtretungsvertrag** (Übertragung 50% Geschäftsanteile, notariell beurkundungspflichtig)

---

## A2. Gründung Fintutto UG (haftungsbeschränkt) -- Notartermin Donnerstag

### Was zum Notar mitbringen:

| # | Dokument / Unterlagen | Status |
|---|----------------------|--------|
| 1 | **Personalausweis** (oder Reisepass) | ☐ |
| 2 | **Musterprotokoll** (vereinfachte Gründung für 1-Personen-UG) -- wird vom Notar bereitgestellt | ☐ |
| 3 | **Unternehmensgegenstand** (Formulierungsvorschlag, s.u.) | ☐ |
| 4 | **Geschäftsanschrift** (Firmensitz) | ☐ |
| 5 | **Stammkapital**: Mindestens 1 EUR, empfohlen **500--1.000 EUR** | ☐ |
| 6 | **Geschäftskonto-Nachweis** (oder nach Gründung einrichten -- Stammkapital einzahlen) | ☐ |

### Empfohlener Unternehmensgegenstand:

> "Die Entwicklung, der Betrieb und die Lizenzierung von Software, insbesondere im Bereich Künstliche Intelligenz, Echtzeit-Übersetzung und digitaler Tourismuslösungen, sowie die Erbringung von IT-Dienstleistungen, Beratung und Cloud-Services."

**Wichtig:** Den Gegenstand breit formulieren, damit Fintutto UG nicht nur GuideTranslator, sondern das gesamte Fintutto-Ökosystem abdecken kann.

### Ablauf beim Notar:

1. Musterprotokoll wird verlesen und unterschrieben
2. Notar reicht beim Handelsregister ein (~2-4 Wochen bis HR-Eintrag)
3. Du bekommst eine **Handelsregisteranmeldung** (beglaubigt)
4. **Geschäftskonto** eröffnen und Stammkapital einzahlen
5. Bis HR-Eintrag firmiert die Gesellschaft als "Fintutto UG (haftungsbeschränkt) i.Gr."
6. Gewerbeanmeldung beim zuständigen Gewerbeamt
7. Fragebogen zur steuerlichen Erfassung ans Finanzamt

### Kosten Notar (Musterprotokoll / 1-Personen-UG):

| Posten | Betrag |
|--------|--------|
| Notargebühren (Beurkundung) | ~60--120 EUR |
| Handelsregistergebühr | ~150 EUR |
| Gewerbeanmeldung | ~20--60 EUR |
| **Gesamt** | **~230--330 EUR** (zzgl. Stammkapital) |

### Was NICHT zum Notar muss:

- Der Lizenzvertrag (Fintutto UG ↔ AI Tour Guide UG) -- das ist ein privatrechtlicher Vertrag, kein notarieller Akt
- Die Vertriebsvereinbarung -- ebenso kein Notar nötig
- Ulrichs Zustimmung -- die Fintutto UG ist DEINE Firma, 100% Alexander

---

## B. Noch erforderlich

### 1. Gesellschafterbeschluss zur Genehmigung des Lizenzvertrags

**Warum:** Alexander ist gleichzeitig Lizenzgeber (Fintutto) UND Geschäftsführer der Lizenznehmerin (AI Tour Guide UG). Das ist ein **Insichgeschäft gemäß § 181 BGB**. Ohne Gesellschafterbeschluss ist der Lizenzvertrag schwebend unwirksam.

**Inhalt:**
- Zustimmung beider Gesellschafter zum Lizenzvertrag
- Befreiung des GF von den Beschränkungen des § 181 BGB für dieses Geschäft
- Genehmigung der Lizenzgebühr von 45%

**Aufwand:** Formular, 1 Seite, muss von beiden unterschrieben werden

---

### 2. Gesellschaftervereinbarung (Ergänzung)

**Warum:** Die bestehende Satzung der AI Tour Guide UG regelt vermutlich nicht alle notwendigen Punkte für die neue Struktur.

**Sollte regeln:**
- **Deadlock-Klausel**: Bei 50/50 braucht ihr eine Pattsituation-Lösung (z.B. Mediator, oder Stichentscheid des GF bei operativen Fragen)
- **Geschäftsführer-Schutz**: Alexander ist nur mit wichtigem Grund abberufbar
- **Zustimmungspflichtige Geschäfte**: Welche Entscheidungen brauchen beide Gesellschafter? (z.B. Aufnahme neuer Gesellschafter, Änderung der Satzung, Geschäfte über X EUR)
- **Drag-Along / Tag-Along**: Falls einer verkaufen will
- **Vorkaufsrecht**: Falls Ulrich seine Anteile verkaufen will
- **Wettbewerbsverbot**: Für beide Gesellschafter
- **Informationsrechte**: Regelmäßige Berichterstattung

**Aufwand:** Anwalt empfohlen, ~1.000--2.000 EUR

---

### 3. Geschäftsführervertrag / -beschluss

**Warum:** Klarstellung, dass Alexander alleiniger GF mit Einzelvertretungsbefugnis ist und bleibt.

**Sollte regeln:**
- Alleinvertretungsbefugnis
- Befreiung von § 181 BGB (generell oder für definierte Geschäfte)
- Vergütung des GF: **2.500 EUR/Monat** (netto) für Verwaltung, Compliance, Haftung und operative Leitung
- Zustimmungsfreie Geschäfte bis [X] EUR
- Nicht abberufbar ohne wichtigen Grund

**Aufwand:** 1--2 Seiten, Notar nur wenn Satzungsänderung nötig

---

### 4. IP-Zuordnungsbestätigung (IP Assignment Confirmation)

**Warum:** Schriftliche Bestätigung, dass der gesamte Fintutto Translator-Code (inkl. Vertriebsmarke "GuideTranslator"), alle Designs, Domains und Marken **Eigentum von Fintutto UG / Alexander Deibel** sind und **nie** Eigentum der AI Tour Guide UG waren.

**Inhalt:**
- Liste aller IP-Assets (Code, Domains, Marken, Designs)
- Bestätigung beider Gesellschafter, dass diese nicht zur UG gehören
- Datum der Erstellung
- Bestätigung, dass die UG keinerlei Ansprüche daran hat

**Aufwand:** 1 Seite, beide unterschreiben

---

### 5. Markenanmeldung "Fintutto Translator" + "GuideTranslator"

**Warum:** Ohne eingetragene Marken kann jeder die Namen nutzen. **Beide** Marken auf Fintutto UG / Alexander anmelden, NICHT auf die AI Tour Guide UG. "GuideTranslator" ist die lizenzierte Vertriebsmarke der UG -- das Eigentum bleibt bei Fintutto.

**Kosten:** ~300 EUR/Marke beim DPMA (deutsches Patent- und Markenamt), ~850 EUR/Marke beim EUIPO (EU-weit)

**Priorität:** Hoch -- beide Marken vor Seatrade Miami anmelden

---

### 6. Datenschutz / DSGVO

**Warum:** Die Software verarbeitet Audiodaten und potenziell personenbezogene Daten.

**Benötigt:**
- **Auftragsverarbeitungsvertrag (AVV)** zwischen Fintutto UG und AI Tour Guide UG
- **Datenschutzerklärung** für die Software / Website
- **Verzeichnis der Verarbeitungstätigkeiten** (Art. 30 DSGVO)

**Aufwand:** Vorlagen verfügbar, anpassen ~1 Tag

---

## C. Rechnungen / Altlasten klären

### Die offenen 270.000 EUR Rechnungen

**Problem:** Alexander hat als Fintutto UG an die AI Tour Guide UG (deren GF er ist) Rechnungen über ~270k EUR gestellt. Ulrich hat das als "Insolvenz-treibend" bezeichnet.

**Optionen:**

| Option | Beschreibung | Empfehlung |
|--------|-------------|------------|
| **Stornieren** | Rechnungen annullieren, Leistung als Vorleistung in die Lizenz einbringen | Sauberste Lösung |
| **Gesellschafterdarlehen** | Umwandeln in nachrangiges Darlehen, tilgbar bei Liquidität | Kompromiss |
| **Stehen lassen** | Rechnungen gelten als gestundet | Maximaler Conflict |

**Empfehlung:** Stornieren und als Dokumentation der geleisteten Arbeit behalten. Die Lizenzgebühr von 35% ist die faire laufende Vergütung für die investierte Entwicklungsleistung.

---

## D. Prioritäten-Reihenfolge

| Prio | Dokument | Wann |
|------|----------|------|
| **1** | **Fintutto UG gründen** (Notar) | **Donnerstag, diese Woche** |
| **2** | Partnerschaftsvorschlag an Ulrich | Sofort (nach Telefonat) |
| **3** | Gesellschafterbeschluss § 181 BGB | Vor Lizenzvertrag-Unterzeichnung |
| **4** | IP-Zuordnungsbestätigung | Zusammen mit Lizenzvertrag |
| **5** | Lizenzvertrag unterzeichnen (Fintutto UG ↔ AI Tour Guide UG) | Nach HR-Eintrag Fintutto UG |
| **6** | Vertriebsvereinbarung unterzeichnen | Nach Einigung |
| **7** | Gesellschaftervereinbarung (Ergänzung) | Innerhalb 4 Wochen (Anwalt) |
| **8** | Markenanmeldung (auf Fintutto UG!) | Vor Seatrade (April) |
| **9** | DSGVO-Dokumente | Vor erstem Kunden |

---

## E. Kosten-Übersicht

| Posten | Geschätzte Kosten |
|--------|------------------|
| Anwalt: Gesellschaftervereinbarung | 1.000--2.000 EUR |
| Anwalt: Prüfung Lizenzvertrag | 500--1.000 EUR |
| Notar (falls Satzungsänderung nötig) | 300--500 EUR |
| Markenanmeldung DPMA | ~300 EUR |
| Markenanmeldung EUIPO (optional) | ~850 EUR |
| **Gesamt (Minimum)** | **~2.100 EUR** |
| **Gesamt (empfohlen)** | **~4.650 EUR** |

**Anmerkung:** Die AI Tour Guide UG sollte diese Kosten tragen -- es sind Geschäftsausgaben für die Strukturierung des Geschäftsbetriebs.

---

*Alle Dokumente sind Entwürfe und sollten vor Unterzeichnung von einem auf Gesellschaftsrecht spezialisierten Rechtsanwalt geprüft werden.*
